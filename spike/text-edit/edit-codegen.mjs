...
// reads back (element.textContent) are the DECODED form — "what's", not the "what&apos;s" that's in
// source. ts-morph hands us the RAW source text with entities intact, so we decode before comparing
// or a perfectly good edit looks like `no editable text matching …`. Single-pass: `&amp;apos;`
// decodes to the literal `&apos;`, matching what a browser shows. Unknown entities are left as-is.
...
// Inverse, for write-back into a JsxText node: & < { MUST be encoded (else the parse corrupts);
// > " ' } are encoded too so a retype never reintroduces a no-unescaped-entities lint error and
// apostrophes round-trip to the `&apos;` the file already used. `&` first, to not double-encode.
function encodeJsxText(s) {
  return s
    .replace(/&/g, "&amp;")
    .replace(/</g, "&lt;")
    .replace(/>/g, "&gt;")
...
function editableTextNodes(sf) {
  ...
  for (const t of sf.getDescendantsOfKind(SyntaxKind.JsxText)) {
    const raw = t.getText();
    const text = norm(decodeEntities(raw)); // compare against the rendered words, not source entities
    ...
    out.push({
      ...
      setText: (next) => {
        // keep leading / trailing whitespace, swap the meaningful core (re-encoded to valid JSX text)
