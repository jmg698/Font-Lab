...
export function agentsBlock() {
  return [
    ...
    "4. **Ship** — `font_lab_apply` writes the exact code for the stack (next/font + Tailwind on Next; self-hosted @font-face through Tailwind @theme / v3 utilities / the project's own font vars elsewhere), reversibly (`font_lab_undo`). It verifies every family is buildable before writing; after applying, run the project's build to confirm and report honestly.",
    "",
