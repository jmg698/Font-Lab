...
export function agentsBlock() {
  return [
    ...
    "",
    "When the user wants to choose, change, compare, or improve the FONTS / typography of this app — any framework: Next.js, Vite, Astro, Remix, SvelteKit, TanStack, … — drive the `font_lab_*` MCP tools. **Do as much of the setup yourself as your environment allows before involving the human — then hand them only what you genuinely can't do.** Route by `font_lab_analyze`'s `capabilities` (`shipNote` names the path): the live panel is Next-only, but every stack still previews on the REAL site (`font_lab_screenshot_directions` — no panel needed, and it starts the dev server itself when none is running) and auto-ships where there's a CSS seam (`font_lab_apply`) — a non-Next stack is a different route, never a reason to stop. The generic specimen sheet (`font_lab_preview`) is a LOCKED last resort: it refuses until a real screenshot attempt has failed on infrastructure, so never lead with it or present `.font-lab/preview.html` as the preview.",
    "",
